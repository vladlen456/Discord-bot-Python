...
async def on_member_join(member):
    ...
    role = discord.utils.get(member.guild.roles, id=configure.config['id'])
...
async def mute(ctx, member: discord.Member):
    mute_role = discord.utils.get(ctx.message.guild.roles, id=configure.config['id2'])
...
Bot.run(configure.config['token'])
